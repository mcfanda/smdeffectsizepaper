...
Here we derive some of the most important formula in the paper. In this document, equations referred to with REF. # are equations of the original paper; the number # is the reference number of the equation in the original paper. 
...
The total expected variance of the $y$ variable will be:
...
### Paper result
In the paper we assert that "if one group has $\mu_1=d$ and all other groups share the same mean $\mu_{2..k}=0$, and the comparison is tested with the contrast $\boldsymbol{c}=\{(k-1),-1_2,-1_3,..,-1_k\}$, the contast value is $c\mu=(k-1)\cdot d$. The g-method yields, after simple algebra, $g={1 / ({k-1})}$, and thus $\delta_{gk}=\delta_{02}$, independently of the number of groups $k$."
